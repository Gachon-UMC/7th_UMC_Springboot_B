...
public class ListReviewRestController {
    ...
    public ApiResponse<ReviewResponseDto.ReviewPreViewListDTO> getReviewList(
        ...
        return ApiResponse.onSuccess(ReviewConverter.reviewPreViewListDTO(storeQueryServiceimpl.getReviewList(storeId,page-1)));
    }


    @GetMapping("/{UserId}/reviews/users")
    @Operation(summary = "특정 가게의 리뷰 목록 조회 API",description = "특정 유저의  리뷰들의 목록을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "acess 토큰 만료",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    public ApiResponse<ReviewResponseDto.ReviewPreViewListDTO> getUserReviewList(
            @ExistUserReview @PathVariable(name = "UserId") Integer UserId,
            @Parameter(description = "조회할 페이지 번호 (1 이상)", example = "1")
            @CheckPage @RequestParam(name = "page") Integer page
    ){
        storeQueryServiceimpl.getUserReviewList(UserId,page-1);
        return ApiResponse.onSuccess(ReviewConverter.reviewPreViewListDTO(storeQueryServiceimpl.getReviewList(UserId,page)));
    }
